...
from irus import IrusInvasion, IrusLadder, IrusMemberList, IrusResources
...
def lambda_handler(event: dict, context: LambdaContext):
    ...
    logger.info(
        f"Downloading file {filename} from {url} to {target} for invasion {name}"
    )
    ...
    try:
        if filename[-4:] != ".png":
            ...
            s3.upload_fileobj(
                pool_mgr.request("GET", url, preload_content=False), bucket_name, target
            )
        ...
        if status == 200:
            ...
            if process == "Ladder":
                ladder = IrusLadder.from_ladder_image(
                    invasion, members, bucket_name, target
                )
            elif process == "Roster":
                ladder = IrusLadder.from_roster_image(
                    invasion, members, bucket_name, target
                )
            else:
