...
def invasion_from_table():
    new_invasion = IrusInvasion.from_user(
        day=2, month=5, year=2024, settlement="ww", win=True
    )
...
def invasion_from_table_item():
    new_invasion = IrusInvasion.from_user(
        day=3, month=5, year=2024, settlement="mb", win=True
    )
...
def test_invasion_from_user_bad_settlement():
    with pytest.raises(ValueError) as excinfo:
        invasion = IrusInvasion.from_user(
            day=1, month=5, year=2024, settlement="xx", win=True
        )
